...
window['ttargetCPA'] = {
    ...
    init: function(){
        ...
        if(ttargetCPA.trackId){
            var track_expires = ttargetCPA.getURLParameter('track_exp') ||  30*24*60*60;
            ttargetCPA.setCookie('trackId', ttargetCPA.trackId, track_expires);
        }else{
    ...
    processClick: function(targetId){
    ...
    loadTargets: function(){
        ...
        tt.setAttribute('src', 'http://tt.ttarget.ru/track.js?track='+ttargetCPA.trackId);
        tt.onreadystatechange = tt.onload = function(){
            if (!tt.readyState || /loaded|complete/.test(tt.readyState)) {
                ttargetCPA.bounceCheck();
                ttargetCPA.attachTargets();
            }
        };
    ...
    bounceCheck: function(){
        if(ttargetCPA['bc'] != null){
            setTimeout(ttargetCPA.loadTargets, ttargetCPA['bc'] * 1000);
        }
    },
                ...
                if(ttargetCPA['targets'].hasOwnProperty(targetId)){
                    ...
                    for (var i = 0; i < elements.length; i++){
                        if(!elements[i].hasAttribute('ttargetCPAAttached')){
                            ttargetCPA.addEvent(elements[i], 'click', function(targetId){ return function(e){ ttargetCPA.processClick(targetId); }; }(targetId));
...
window['ttargetCPA'].action = ttargetCPA.processClick;
window['ttargetCPA'].init();