...
class EImageValidator extends CFileValidator{
    ...
    protected function validateFile($object, $attribute, $file)
    {
        ...
        if($this->maxWidth !== null && $size[0] > $this->maxWidth){
            $message=$this->tooLargeWidth ? $this->tooLargeWidth : Yii::t('yii','Photo should be at max {width}px in width');
        ...
        if($this->maxHeight !== null && $size[1] > $this->maxHeight){
            $message=$this->tooLargeHeight ? $this->tooLargeHeight : Yii::t('yii','Photo should be at max {height}px in height');
